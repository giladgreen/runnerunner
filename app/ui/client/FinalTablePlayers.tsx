...
export default async function FinalTablePlayers({title, params}: { title: string,  params: {userId: string } }) {
    const date = (new Date()).toISOString().slice(0, 10);
    const content = await getFinalTablePlayersContent(date, false, params.userId) as JSX.Element;
