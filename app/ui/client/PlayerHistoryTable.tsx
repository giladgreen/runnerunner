...
export default async function PlayerHistoryTable({
  ...
  return <div style={{ width: '100%'}}>
      {historyLog.map(log => {
        return <div key={log.id} style={{ width: '100%', padding: 20, border: '1px solid gray', borderRadius:10, marginBottom:10}}>
            <div  className="flex ">
                <div style={{marginLeft:3}}> שינוי:</div>
                <div style={{marginLeft:30}}> {formatCurrency(log.change)}</div>
            ...
            <div>
                ...
                <div>{formatDateToLocal(log.updated_at)}, {getTime(log.updated_at)}</div>
